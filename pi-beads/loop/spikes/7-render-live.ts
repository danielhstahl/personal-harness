 ...
 * not one escape byte.
 *
 * It bootstraps itself: `tsx spikes/7-render-live.ts` is the driver; the driver
 ...
 *
 * Run: `npx tsx spikes/7-render-live.ts`
 */
...
import type { RunnerEvent } from "../src/agent.js";
import { createWorkPresenter } from "../src/render.js";
...
function runPtyCase(name: string, captureFile: string): Promise<CaseResult> {
  const command = `${shq(process.execPath)} ${shq(tsxCli)} ${shq(self)} child ${shq(name)}`;
...
function runPlainCase(): Promise<CaseResult> {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [tsxCli, self, "child", "plain"], {
...
async function drive(): Promise<void> {
  ...
  lines.push("");
  lines.push(
    'generated by: script -q -f -c "tsx spikes/7-render-live.ts child <case>" <capture>',
  );
  lines.push("");
  lines.push("## checks");
